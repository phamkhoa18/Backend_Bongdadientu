...
const NewsController = {
    ...
    List_new_show : async(req,res) => {
        try {
            const list = await News.find({show : true}).populate('category_id').populate('type_id').sort({ updated_at: -1 }); ;
            res.status(200).json(list) ;
        } catch (error) {
            res.status(404).json(error);
        }
    ...
    NewFeed : async(req,res) => {
        try {
            const list = await News.find().sort({ updated_at: -1 }).populate('type_id').limit(5) ;
            res.status(200).json(list) ;
        } catch (error) {
            res.status(404).json(error);
        }
